...
model = T5ForConditionalGeneration.from_pretrained(model_name, cache_dir="C:/Models")
model.eval()
model.to(device)
tokenizer = T5Tokenizer.from_pretrained(model_name, cache_dir="C:/Models")

# Function to extract audio from video and convert it to text
def extract_text_from_video(video_path, output_dir=r'C:\Файлы\Meznar-hakaton\video-test-audio'):
...
def summarize_video(video_path):
    ...
    brief_summary = detect_language_and_summarize(extracted_text)
    print("\nBrief Summary:", brief_summary)

    big_summary = detect_language_and_summarize(extracted_text)
